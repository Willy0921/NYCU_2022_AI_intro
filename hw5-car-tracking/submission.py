...
from util import Belief, colToX, rowToY, xToCol, yToRow
...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                u = math.sqrt((agentY - rowToY(row)) ** 2 +
                              (agentX - colToX(col)) ** 2)
                p = util.pdf(u, Const.SONAR_STD, observedDist)
                self.belief.setProb(
                    row, col, p * self.belief.getProb(row, col))
    ...
    def elapseTime(self) -> None:
        ...
        new_belief = util.Belief(
            self.belief.getNumRows(), self.belief.getNumCols(), value=0)
        for oldTile, newTile in self.transProb:
            new_belief.addProb(newTile[0], newTile[1], self.belief.getProb(
                *oldTile) * self.transProb[(oldTile, newTile)])
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        observe_distribution = collections.defaultdict(float)
        for row, col in self.particles:
            u = math.sqrt((agentY - rowToY(row)) ** 2 +
                          (agentX - colToX(col)) ** 2)
            p = util.pdf(u, Const.SONAR_STD, observedDist)
            observe_distribution[(row, col)] = self.particles[(row, col)] * p
        new_particles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            particle = util.weightedRandomChoice(observe_distribution)
    ...
    def elapseTime(self) -> None:
        ...
        new_particles = collections.defaultdict(int)
        for particle_location in self.particles:
            if particle_location in self.transProbDict:
                for _ in range(self.particles[particle_location]):
                    tile = util.weightedRandomChoice(
                        self.transProbDict[particle_location])
                    new_particles[tile] += 1
        self.particles = new_particles
        return
        raise Exception("Not implemented yet")
        # END_YOUR_CODE
