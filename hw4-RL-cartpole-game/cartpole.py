...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        array = np.arange(lower_bound, upper_bound,
                          (upper_bound - lower_bound) / num_bins)
        return array[1:]
        # End your code
    ...
    def discretize_observation(self, observation):
        ...
        state = [self.discretize_value(
            observation[index], self.bins[index]) for index in range(4)]
        return state
        # End your code
    ...
    def choose_action(self, state):
        ...
        if p < self.epsilon:
            ...
            action = np.argmax(
                self.qtable[state[0], state[1], state[2], state[3]])
        return action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if done:
            ...
            self.qtable[state[0]][state[1]][state[2]][state[3]][action] = (
                1 - self.learning_rate) * self.qtable[state[0]][state[1]][state[2]][state[3]][action] + self.learning_rate * (reward + self.gamma * max_Q(next_state))
        # End your code
        ...
        def simple_test(env, cur_qtable):
            ...
            rewards = []
            for _ in range(10):
                state = testing_agent.discretize_observation(
                    testing_agent.env.reset())
                count = 0
                while True:
                    count += 1
                    action = np.argmax(testing_agent.qtable[tuple(state)])
                    next_observation, _, done, _ = testing_agent.env.step(
                        action)
                    next_state = testing_agent.discretize_observation(
                        next_observation)
                    if done == True:
        ...
        if done:
            ...
            global best_avg_rewards
            mean_test_rewards = np.mean(test_rewards)
            if mean_test_rewards > best_avg_rewards:
                best_avg_rewards = mean_test_rewards
