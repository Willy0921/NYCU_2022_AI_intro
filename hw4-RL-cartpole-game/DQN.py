...
def test(env):
    ...
    for _ in range(100):
        ...
        while True:
            count += 1
            # print("torch.FloatTensor(state) ", torch.FloatTensor(state).shape)
            Q = testing_agent.target_net.forward(
                torch.FloatTensor(state)).squeeze(0).detach()
            # print("torch.FloatTensor(state).squeeze(0) ",
            # torch.FloatTensor(state).squeeze(0).shape)
            # print("torch.FloatTensor(state).squeeze(0).detach() ",
            # torch.FloatTensor(state).squeeze(0).detach().shape)
            # print("Q ", Q.shape)
